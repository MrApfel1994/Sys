...
class Delegate<R(Args... args)> {
    ...
    static R method_stub(void *p_object, Args... args) {
        T *p = static_cast<T *>(p_object);
        return (p->*TMethod)(args...);
    }

    template<class T, R(T::*TMethod)(Args... args) const>
    static R const_method_stub(void *p_object, Args... args) {
        const T *p = static_cast<T *>(p_object);
        return (p->*TMethod)(args...);
    }

    template<R(*TMethod)(Args... args)>
    static R function_stub(void *p_object, Args... args) {
        return (*TMethod)(args...);
    }
